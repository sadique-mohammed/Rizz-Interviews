...
export default function StreakCard({ streakData, className }: StreakCardProps) {
  ...
  const boxClasses =
    'group flex items-center justify-between rounded-md border border-gray-200 bg-white px-3 py-2 transition-all hover:border-brand hover:bg-brand/8 hover:shadow-sm cursor-default';
      ...
      <CardContent className='p-2'>
        <div className='grid grid-cols-1 md:grid-cols-3 gap-2'>
          <div className={boxClasses}>
            <div className='flex items-center space-x-2.5'>
              <div className='flex-shrink-0'>
                <div className='w-8 h-8 rounded-md flex items-center justify-center bg-brand/12'>
                  <Flame className='h-4 w-4 text-brand' />
                </div>
              </div>
              <div className='flex flex-col justify-center'>
                <div className='flex items-baseline gap-1'>
                  <span className='text-lg font-bold text-brand leading-none tracking-tight'>
                    {currentStreak}
                  </span>
                  <h4 className='font-medium text-gray-900 text-xs'>days Streak</h4>
                </div>
              </div>
            </div>
          ...
          <div className={boxClasses}>
            <div className='flex items-center space-x-2.5'>
              <div className='flex-shrink-0'>
                <div className='relative w-8 h-8 flex items-center justify-center bg-brand/12 rounded-md'>
                  <svg className='w-full h-full transform -rotate-90 absolute inset-0'>
                    <circle
                      ...
                      strokeWidth='3'
                      fill='transparent'
                      className='text-gray-200 group-hover:text-brand/20 transition-colors'
                    />
                    ...
                    <circle
                      ...
                      strokeDashoffset={
                        2 * Math.PI * 13 - (progressPercent / 100) * (2 * Math.PI * 13)
                      }
                      strokeLinecap='round'
                      className='text-brand transition-all duration-1000 ease-out'
                    />
                  </svg>
                  <Target className='absolute w-3.5 h-3.5 text-brand' />
                </div>
              </div>
              <div className='flex flex-col justify-center'>
